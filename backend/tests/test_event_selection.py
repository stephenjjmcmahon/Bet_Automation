...
def test_selects_correct_event_from_candidates():
    mock_resp = _mock_openai_response('{"event_id": "111"}')
    with patch("backend.services.ai_interpreter.client.chat.completions.create", return_value=mock_resp):
        result = AIInterpreter.select_event("Back Arsenal vs Burnley 10", CANDIDATES)
    assert result == "111"
...
def test_returns_none_for_empty_candidates():
    ...
    with patch("backend.services.ai_interpreter.client.chat.completions.create") as mock_api:
        result = AIInterpreter.select_event("Back Arsenal 10", [])
    assert result is None
    mock_api.assert_not_called()


def test_returns_none_when_ai_finds_no_match():
    mock_resp = _mock_openai_response('{"event_id": null}')
    with patch("backend.services.ai_interpreter.client.chat.completions.create", return_value=mock_resp):
        result = AIInterpreter.select_event("Back Arsenal in the FA Cup Final 10", CANDIDATES)
    assert result is None


def test_returns_none_when_ai_returns_empty_content():
    mock_resp = _mock_openai_response(None)
    with patch("backend.services.ai_interpreter.client.chat.completions.create", return_value=mock_resp):
        result = AIInterpreter.select_event("Back Arsenal 10", CANDIDATES)
    assert result is None
...
def test_all_candidates_are_included_in_prompt():
    ...
    with patch("backend.services.ai_interpreter.client.chat.completions.create", return_value=mock_resp) as mock_api:
        AIInterpreter.select_event("Back Arsenal vs Burnley 10", CANDIDATES)
...
def test_user_input_is_included_in_prompt():
    mock_resp = _mock_openai_response('{"event_id": "111"}')
    with patch("backend.services.ai_interpreter.client.chat.completions.create", return_value=mock_resp) as mock_api:
        AIInterpreter.select_event("Back Arsenal vs Burnley tonight 10", CANDIDATES)
